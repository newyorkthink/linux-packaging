...
| 范围 | 主要内容 |
| --- | --- |
| 文首永久规则 | 修复记录只增不删、linuxdeploy 必须完整遵守 `linuxdeploy_projects.md`、测试代码边界与必要验证、修改后及时提交、公共下载与安装入口、Git 分支、撤销提交、仓库独立运行、版本清单即时写入 |
...
- 对应应用 README、问题记录、`PENDING_AI_TASKS.md` 或其他专门历史文档中，已经写明“出现过什么问题、当时看到什么错误、尝试了什么、为什么失败、最终怎么修、哪些内容仍未验证”的记录必须保留。后续修复只能新增新的记录说明当前结论，不能删掉旧记录来让历史看起来只有最终方案。
