...
**AI 可以根据当前修改的风险执行必要且相关的语法检查、静态分析、临时试构建、试打包、启动检查或功能验证；但验证应在本地、容器或临时目录中完成，不得为了验证当前修改而向正式提交加入测试脚本、冒烟逻辑、测试专用代码、test workflow、测试 Job 或测试 Step。**
...
- 禁止为了验证打包修改，在 `build_*.sh`、`AppRun`、wrapper、helper、源码、Makefile、CMake、package script 或其他正式文件中加入 unit test、integration test、E2E test、regression test、smoke test、startup test、health check、自测代码或等价逻辑。
- 禁止新增临时或长期的 `test workflow`、测试 Job、测试 Step；禁止为了验证构建而创建 `.github/workflows/*test*.yml`、`smoke` Job或等价 CI 入口。
- 禁止把类似 `timeout ... AppRun`、`xvfb-run ... AppRun`、`SMOKE_RC=...`、`smoke.log` 判断、启动若干秒后根据退出码判断成功等临时冒烟逻辑写入正式构建代码。
...
```bash
# 在空目录首次运行 linuxdeploy，只创建 AppDir 基础目录
...
- linuxdeploy 两个阶段都保留 `--output appimage`。第一次只初始化空 AppDir；当前版本因没有 desktop 会在目录创建后返回 1，脚本必须核对三个基础目录和空文件状态后才能接受。写入应用和完整根 AppRun 后的第二次用于执行当前技术栈插件并生成中间 AppImage；AppRun 是否自动包装以插件实际产物为准。
...
- 第一次必须在空目录执行普通 `linuxdeploy --appdir AppDir --output appimage`，只由 linuxdeploy 创建 AppDir 基础目录；应用文件、desktop、icon、AppRun 和对应命令参数都在该次初始化完成后准备。
...
- 不得把 `test_*.sh`、`verify_*.sh`、`smoke_*.sh`、测试 harness、fixture、mock、冒烟逻辑或等价测试文件加入本次正式提交。
...
- 为验证当前修改而向正式提交新增测试脚本、冒烟逻辑、test workflow、测试 Job、测试 Step 或测试专用代码。

## 12. 用户明确指令优先

**本节不适用于“永久规则：允许必要验证，禁止向提交内容堆入测试代码（不可豁免）”。用户要求验证时可以执行必要测试，但不得把只为验证当前修改而添加的测试脚本、smoke test、test workflow、测试 Job、测试 Step 或临时测试代码纳入正式提交。**
