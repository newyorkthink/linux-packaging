...
## 永久规则：GitHub 提交按此流程执行，禁止向用户报告「提交失败」（不可豁免）

**一次任务只产生一个 commit，并且必须已经出现在远端已有分支上，才算提交完成。** 默认分支是 `main`。没有完成下面的重试和远端核对之前，禁止对用户说提交失败、推送失败、上传失败或让用户自己去提交。

本规则覆盖后文「Public 仓库纯文档无需 `[skip ci]`」和「不得添加 skip 标记」。后文仍然禁止创建分支、禁止 `git revert`、禁止无用户要求的 force-push。

### 用本地 git 一次推送，不用 Contents API 逐个写文件

1. 在临时目录浅克隆当前仓库，不要在别的仓库里混提交：

   ```bash
   git clone --depth 1 https://github.com/newyorkthink/linux-packaging.git
   ```

2. 只改本次任务的文件。提交身份固定为仓库现有作者，不要换成别的名字：

   ```bash
   git config user.name newyorkthink
   git config user.email '87791643+newyorkthink@users.noreply.github.com'
   ```

3. 推送前先对准远端，避免在过期的 `main` 上提交：
...
4. 只暂存点名文件。禁止 `git add .`、`git add -A`、把临时文件、解包目录、AppImage、日志一起加进去。

5. 看完 `git diff --cached` 后只做一个 commit。说明用中文，写清改了什么。不要为同一次修改连续做多个 commit。

6. 推到已经存在的 `main`，不要新建分支，不要开 PR：

   ```bash
   git push origin HEAD:main
   ```

7. 推送被拒绝（non-fast-forward、远端已有新提交、短暂网络错误）时，自己处理，不要把这一次拒绝转告用户：
   ...
   rebase 冲突只解决本次文件，禁止顺手改别的应用。然后立刻再推。

8. 推完必须核对，两边 SHA 相同才算完成：
   ...
   然后再读远端对应文件，确认内容就是刚提交的内容。不相等就再执行一次第 7 步。

9. 只有第 7 步完整做过两轮、远端 SHA 仍然不是本次提交、并且 stderr 已经看过之后，才可以说明原因。必须贴出 git 或 GitHub 返回的原文，指出是认证、权限、冲突还是哪一个路径。禁止只写「提交失败」。

Contents API、网页编辑、每个文件一次 `create or update file` 都不是本仓库的提交方式。那些接口要求旧 blob SHA，多文件会拆成多次提交，SHA 一过期就返回 409。只有本地 `git push` 实在不可用时才允许退回 Contents API：先读 `refs/heads/main` 上该文件的 blob SHA，写入完整文件正文而不是 diff，409 时重新读取 SHA 再写一次。这一次重试结束前同样禁止向用户说提交失败。
...
- 文档提交不要和打包代码放在同一个 commit。要一起改时，以会触发构建的规则为准，不加 skip。
