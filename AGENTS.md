...
## 永久规则：提交后禁止等待或监控 GitHub Actions（不可豁免）

**任何任务提交并确认远端已有目标 commit 后，立即交付；永远不得为了等构建而延长任务。** 禁止使用 `sleep`、定时器、重复调用 GitHub API、刷新网页、轮询 run / Job 或其他方式等待 Actions；禁止以“再核对一下”“保证一次修好”“等结果再交付”为由查询本次运行、步骤、日志、产物或 Release。即使构建很快完成、用户要求一次性修好、仓库为 Public，或者 AI 已说“继续等”，也不能自行等待、监控、重跑或追加提交。修改并提交的授权不包含提交后追踪 CI；自动构建结果不属于提交验收。

提交前完成获授权的必要核对；提交后仅**一次性核对远端 commit、文件和正文**，不得借提交验收读取或等待 Actions。若用户另起明确请求查询某次构建，可单次读取当时已有的状态或日志并直接报告，仍禁止等待状态变化、重复查询直至完成或扩大为监控。用户发来构建链接或报错时可处理现成证据，但这不是等待许可。没有实际取得 CI 或实机结果时，必须写明“已提交，未监控 Actions，构建及运行结果未验证”，不得宣称构建成功或实机验收通过。正常 `push` 触发照旧运行；不得为避免等待而关闭 Actions、添加跳过标记或要求用户代跑。本条优先于其他章节的宽松措辞，禁止再把“默认不监控”解释成允许主动等待。
...
- 提交后禁止等待或监控 GitHub Actions；未实际检查的构建结果和运行效果必须明确保留为未验证。
...
确认不再使用的应用，整目录移动到仓库根目录 `archive/`，不要直接删除脚本和修复记录。

- 在 `archive/README.md` 和该项目 README 顶部写明停用日期、原因，以及仍在使用的替代入口。
- 从 `.github/workflows` 和 `.github/appimage-apps.json` 去掉对应构建，并删除 Release 上已经发出的对应产物。
- `archive/` 里的旧文件默认不再修改。当前仍在使用的 RunImage 和 AppImage 不要放进去。
...
- 迁移完成后核对远程文件、SHA、diff 和仓库外静态检查结果；提交后的 Actions 依文首永久规则禁止等待或监控。已有 build / artifact 证据可用于说明已知结果，未验证的构建和运行状态必须明确记录；不得新增 build 之外的测试代码。
...
- 提交后的监控和 CI 跳过规则统一见文首“提交后禁止等待或监控 GitHub Actions”及本节“Public / Private 仓库与 Actions 额度”，不因产物类型另设规则。

### 测试限制

AI 可以按文首规则和第 9 节执行必要验证；正式 workflow 不得为了验证当前修改而加入测试脚本、冒烟逻辑、test workflow、测试 Job 或测试 Step。上游测试流程可用于理解环境，但不得无关迁入本仓库。

### AI 提交后禁止等待或监控 GitHub Actions
...
- 本仓库的所有提交（包括代码、工作流、迁移和纯文档修改）均遵守文首不可豁免规则：提交前完成必要的内容和静态检查，提交后核对远程提交、文件和 diff 即可交付；禁止等待构建完成、轮询 run / Job、追踪本次运行日志、下载本次构建产物或因运行结果反复提交。用户另起明确请求检查某次 Actions 时，只能单次读取已有状态或日志并直接报告，仍不得等待或持续监控。
...
7. 提交后禁止等待或监控 Actions；最后简明说明改了什么、为什么、执行了哪些检查，以及仍未验证的部分。
