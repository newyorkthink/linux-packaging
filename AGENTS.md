...
本规则覆盖后文「Public 仓库纯文档无需 `[skip ci]`」和「不得添加 skip 标记」。后文仍然禁止创建分支、禁止 `git revert`、禁止无用户要求的 force-push。
...
- 这次 diff 只要含构建脚本、workflow、应用清单或 `common/` 里的脚本，提交说明里禁止写 `[skip ci]`、`[ci skip]`、`[no ci]`、`[skip actions]`、`[actions skip]`。推送到 `main` 就是构建，不要再手动 `workflow_dispatch` 一次。
- 这次 diff 只有 Markdown、并且没有任何打包代码时，提交说明必须带 `[skip ci]`。否则应用目录里的一篇记录也会把该应用重新打包。
- 同一任务同时修改文档和打包代码时，只做一个 commit；按会触发构建的规则处理，不加 skip。
