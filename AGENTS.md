...
- 如果源仓库脚本存在应用版本硬编码，迁移时仍必须遵守本文件“应用版本必须动态获取”的规则；“原仓库这样写”不是继续写死版本的理由，除非确有版本固定原因。
...
本仓库默认用于持续构建当前最新稳定版软件。除非用户明确要求固定历史版本，或者存在经过说明的兼容性 / 补丁约束，否则构建脚本不得把应用版本号或带版本号的下载地址永久写死在仓库中。
...
- 只有用户明确要求某个固定版本、上游没有可靠的动态版本入口，或当前项目的补丁只兼容某个明确版本时，才允许固定应用版本；此时必须在脚本中说明固定原因，不能伪装成“自动最新版”。
