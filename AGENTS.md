...
| 范围 | 主要内容 |
| --- | --- |
| 文首永久规则 | 测试代码与测试命令、修改后立即提交、Git 分支、撤销提交、仓库独立运行、版本清单即时写入 |
...
## 永久规则：禁止任何测试代码和测试命令（不可豁免）

**AI 永久禁止在本仓库新增、生成、补写、插入、恢复、迁移或建议提交任何测试、冒烟测试或验证代码；同时永久禁止在修改前后、本地、容器、临时目录、GitHub Actions 或任何其他环境执行测试、试跑、构建试跑、语法检查、静态分析、冒烟、验证或等价命令。此规则优先于本文件其他任何规则，也不允许被当前或未来任何用户请求、故障排查需要、CI 需要、兼容性需要或“临时验证”理由覆盖。**
...
- 明确禁止执行 `bash -n`、`shellcheck`、lint、parser、dry-run、测试构建、试打包、AppImage 解包验证、`ldd` 验证、程序启动验证、功能验证及其他任何以检查修改结果为目的的命令。
- 明确禁止把 `test`、`[`、`[[`、`grep`、`timeout`、`ldd`、解包或启动程序等命令作为产物验证、功能验证或测试断言写入构建脚本；生产构建本身必需的架构判断、上游元数据判断、下载失败处理和官方摘要校验不属于测试。
- 禁止在提交后监控、轮询、重跑或额外触发 GitHub Actions 来验证本次修改；正常 `push` 自动触发的正式构建不需要关闭，但 AI 不得跟踪其结果。
...
- AI 只能被动读取用户已经提供的真实运行输出、截图、既有日志和既有产物信息以理解问题；不得对当前修改执行任何测试、验证或静态分析命令。
- 本条不授权 AI 顺手批量删除与当前任务无关的历史文件；但凡当前任务触及的文件中存在由 AI 新增或恢复的测试代码，必须删除，不得继续保留。

## 永久规则：修改后立即提交并推送（不可豁免）

**AI 对本仓库完成用户要求的最小修改后，必须立即把本次精确范围的文件提交并推送到当前已有分支；不得在修改与提交之间执行任何测试、验证、语法检查、静态分析、试构建或试打包命令。**

强制要求：

- 只暂存本次任务明确修改的文件，不得使用会混入无关改动的宽泛暂存。
- 修改完成后直接执行精确暂存、提交和推送，不得以“先测试一下”“先跑静态检查”“先试构建”为理由延迟提交。
- 只能提交到当前已有分支，通常为 `main`；不得创建任何分支。
- 推送失败时只处理认证、网络或远端并发等推送阻塞，不得转而执行测试或试构建。
- 提交后不得监控 GitHub Actions；构建结果和实际运行效果统一保留为未验证，等待既有正式流程或用户真实运行反馈。
...
仓库中的所有操作必须遵守文首“永久规则：禁止任何测试代码和测试命令”。修改完成后直接提交并推送，不得执行任何用于确认修改结果的命令。

仅允许在修改前被动阅读已经存在的证据以理解问题，例如：

- 读取已有 GitHub Actions 正常 build Job 的状态和日志；
- 读取已有 artifact、Release 资产、Git tree、diff、文件大小和 SHA；
- 依据用户提供的真实运行输出、截图或报错核实实际问题；

这些既有证据只用于定位问题，不授权对当前修改执行测试、静态分析、语法检查、构建试跑、产物验证或运行验证。

禁止加入或执行：

- `bash -n`、`shellcheck`、lint、parser、dry-run 或等价静态检查；
- 本地、容器、临时目录或 CI 中的测试构建、试打包和试运行；
- AppImage 解包检查、`ldd` 检查、启动检查和功能检查；
...
如果现有证据不足，应明确说明仍未确认的部分并等待正式构建或用户真实运行反馈；**不得通过任何测试代码、测试命令或验证命令补足证据。**
...
3. 采用最小变更实现。
4. 完成最小修改后立即精确暂存、提交并推送，不执行任何测试或验证命令。
5. 只使用修改前已经存在的日志、产物、元数据和用户真实反馈定位问题；**绝不新增或执行任何测试。**
6. 在对应应用 README 中记录检查或修复结果；已修复条目按第 2 节用修复记录替换对应检查记录。
7. 提交后不监控 Actions、不验证产物；最后简明说明改了什么、为什么，以及构建和运行结果未验证。
