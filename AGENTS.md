...
## 永久规则：修改前告知并锁定范围（不可豁免）

AI 在回答问题、诊断故障或提出修改方案时，必须先核实实际问题、已验证的基线和相关上下文。用户要求修改时，在首次写入文件之前，必须明确告知用户本次要解决的具体问题，以及预计修改的文件夹、文件和每个文件的修改内容；文件须逐一列出仓库相对路径，不能只写笼统目录或“相关文件”。用户要求“先说”“只看”“别改”时，只能做只读检查，不得修改、暂存、提交或推送。

告知后，修改范围仅限已列明且与用户请求直接相关的问题和文件。读取某个文件不等于获准修改它；不得以顺手修复、统一整理或确保完整为由扩大范围。发现必须增加问题、文件或目录，或者需要改变公共函数、工作流、参数、调用方式等超出原清单的内容时，必须先核实原因与影响，并在修改前告知新增的具体问题及文件路径，取得用户对扩大范围的明确授权。不得先改后报。

提交前必须逐项检查完整 diff，对照事先告知的清单排除越界改动；只暂存、提交和推送清单内的文件。完成后如实说明实际修改的文件和问题，不得把检查过但未修改的文件算作修改成果。
