...
VERSION="$("$SCRIPT_DIR/../common/github/download_latest_stable_release_asset.sh" \
  "<owner/repo>" '<资产名-{version}.扩展名>' "<输出文件>")"

# 下载 DEB 时可追加包名和架构，由同一公共入口统一核对 DEB 身份与目标架构
VERSION="$("$SCRIPT_DIR/../common/github/download_latest_stable_release_asset.sh" \
  "<owner/repo>" '<资产名-{version}.deb>' "<输出 DEB>" "<DEB 包名>" "<DEB 架构>")"
