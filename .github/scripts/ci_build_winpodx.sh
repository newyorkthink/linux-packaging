...
# 不安装 FreeRDP，也不安装那 8 个 Qt/XCB 私有库：两者都必须来自打过补丁的官方 AppImage。
...
input_dir="$GITHUB_WORKSPACE/winpodx/dist"
work_dir=/tmp/winpodx-check
mkdir -p "$work_dir"
cp "$input_dir/winpodx.AppImage" "$work_dir/winpodx.AppImage"
...
grep -F "source=official-appimage" \
  "$input_dir/winpodx-release-version.txt"
