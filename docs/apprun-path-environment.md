...
这个脚本只用于首次打包或最终目录仍不确定的项目，调用位置是：**第二次 linuxdeploy 完成之后、最终 appimagetool 封装之前**。首次成品尚未由用户确认时必须保留该调用，不能因为构建成功、静态检查通过或整理结果看起来合理就提前删除。
...
- 用户确认首次成品没有问题后，应从当前仓库 Release 成品的实际解包结果或用户提供的最终成品解包截图中核对真实目录与路径型 export，把确认过的 export 原样固化回对应构建脚本的根 AppRun，并删除该项目的整理脚本调用；不能让已经确定的路径在每次构建中继续自动猜测。

因此公共脚本负责“根据最终 AppDir 校准路径”，而每个应用自己的 AppRun 仍负责决定“这个应用到底需要哪些变量和特殊设置”。

## 从临时整理转为稳定 AppRun

首次生成成品时先保留 `normalize_apprun_paths.sh` 调用。用户确认成品没有问题后，可以下载当前仓库 Release 的最终 AppImage 并在仓库外临时目录解包，也可以使用用户提供的最终成品解包截图。两种证据都要按实际产物核对入口和各项 export：官方插件生成 hook 时核对顶层 `AppRun`、`AppRun.wrapped` 与 `apprun-hooks/` 的关系，没有 hook 时核对完整的顶层 `AppRun`。截图必须能够看清相关目录和完整 export，信息不完整时不得补猜。路径存在只是第一步，还必须确认目录内容与变量用途一致：
...
确认完成后，应把 `AppRun.wrapped`（无 wrapped 时为 `AppRun`）中已经核对的路径型 export 原样写进项目构建脚本生成的根 AppRun，并在同次修改中移除该项目对公共整理脚本的调用、同步应用 README。存在官方插件 hook 时，顶层 hook AppRun 仍由 linuxdeploy 生成，不得抄回构建脚本；没有 hook 时不得人工补造。后续不得仅因通用模板变化而重写已经确认有效的 AppRun。
