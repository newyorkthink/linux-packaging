...
强制规则以 [AGENTS.md](../AGENTS.md) 为准。某个应用的遗留事项、打包方式、稳定基线和修复历史以该应用目录的 `README.md` 为准，不要记在根 README。

## 文档怎么分工

| 文档 | 用途 | 是否保存具体应用细节 |
| --- | --- | --- |
| 根目录 `README.md` | 仓库入口和手动构建入口 | 不再写待办、故障和修复记录 |
...
1. 阅读该应用目录 README 里的搁置说明，确认这次任务没有被明确要求先停住。
...
- 暂时无法解决：只在该应用 README 记录现状和不要重复的方向，不要再往根 README 加索引。
...
- [ ] 第一次空 AppDir 初始化只单行调用 `common/linuxdeploy/initialize_appdir.sh`，没有在应用脚本重复命令。
