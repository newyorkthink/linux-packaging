...
- AppImage 无法使用官方 deb 针对固定安装路径提供的 AppArmor userns 规则，因此便携入口使用 `--no-sandbox --disable-setuid-sandbox` 启动 Electron/Chromium；官方 Codex 二进制及其内置 Linux 命令沙箱保持原样。
- 构建后会检查最终 AppImage 的关键资源、动态库和官方 Codex 二进制摘要，并在隔离 HOME、D-Bus 与 Xvfb 中执行启动测试。
