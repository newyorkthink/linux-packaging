...
- `remotedesktopmanager`：去掉 `LD_LIBRARY_PATH` 后，Linux 实机已确认工具 → 终端能 spawn 宿主 `/bin/sh` 并出现 `$` 提示符，`__pointer_chk_guard` 已消失。Fcitx5 选词时 `^[[D` / `^[[3~` 仍泄漏进 PTY；该终端是 Avalonia TermControl / LocalTerm，不是 VTE，GTK IM 模块滤不了。不得为这个回退已验证基线，也不得再改打包脚本赌 GTK 路径。详见 [remotedesktopmanager/README.md](./remotedesktopmanager/README.md)。
