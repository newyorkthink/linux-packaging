...
- `jriver`：2026-09-18 终端 GUI 已确认。Rofi `run` 清锁后仍无窗口；包内有 `DISPLAY=:0`，但继承 `TERM=linux` 和 `XDG_SESSION_TYPE=tty`。已在 `Run.rcfg` 覆盖这两项，待新产物验证。不得再做 AppRun 包装或 Sharun/GVFS/D-Bus/`LD_PRELOAD` 环境清理。Fcitx5 实际中文输入、网页音频和影院模式仍未验证。详见 [RunImage + quick-sharun 路线记录](./jriver/README_runimage_quick.md)；[旧版稳定路线记录](./jriver/README.md) 继续独立保留。
- `remotedesktopmanager`：AppImage 当前可以正常打包，ICU 与多语言界面可用。已把错误的 `GTK_IM_MODULE=fcitx5` 改为 GTK3 实际模块 ID `fcitx`，并在启动时按 AppImage 挂载路径生成 Fcitx5 `immodules.cache`。已从 `.env` 去掉 `LD_LIBRARY_PATH`，避免内置终端把宿主 `/bin/sh` 绑到包内 `libc.so.6` 上出现 `__pointer_chk_guard` / `GLIBC_PRIVATE` 错误。本地 shell 是否能真正打开、以及候选按键 `^[[A` / `^[[D` 泄漏是否消除，仍待 Linux 实机验证；详见 [remotedesktopmanager/README.md](./remotedesktopmanager/README.md)。
