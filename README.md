...
- `remotedesktopmanager`：去掉 `LD_LIBRARY_PATH` 后，Linux 实机已确认工具 → 终端能 spawn 宿主 `/bin/sh`，`__pointer_chk_guard` 已消失。选词键泄漏的原因是官方 `Devolutions.TerminalControl` 不请求 Avalonia IME 客户端。已加 `DOTNET_STARTUP_HOOKS` 补客户端，待新 AppImage 实机验证。不得回退 ICU、glycin-ng、WebView、`GTK_IM_MODULE=fcitx` 或重新写入 `LD_LIBRARY_PATH`。详见 [remotedesktopmanager/README.md](./remotedesktopmanager/README.md)。
