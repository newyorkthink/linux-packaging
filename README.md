...
- `jriver`：2026-09-13 按要求将旧入口原样保留为 `build_jriver_legacy_20260913.sh`，新 `build_jriver.sh` 使用标准 RunImage + quick-sharun，尚未验证构建与实机功能。旧版 GUI 已可用、影院模式鼠标点击卡住等记录继续保留；不把历史问题归因于 Rofi，也不将旧版 CEF、音频和路径补丁移入新入口。后续需确认新产物的 GUI、文件选择器、影院模式、网页音频和中文输入，不能把更换打包方式视为已经修复。详见 [jriver/README.md](./jriver/README.md) 第 14 节。
