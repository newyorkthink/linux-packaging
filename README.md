...
- `remotedesktopmanager`：libc 泄漏已实机修好。含 Avalonia IME 钩子的新 AppImage 已上传 `latest`（SHA-256 `8abf7cc9…`）。选词键是否还漏，待这次实机验证；若仍漏，停止再改打包脚本。不得回退 ICU、glycin-ng、WebView、`GTK_IM_MODULE=fcitx` 或重新写入 `LD_LIBRARY_PATH`。详见 [remotedesktopmanager/README.md](./remotedesktopmanager/README.md)。
