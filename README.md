...
- `remotedesktopmanager`：AppImage 当前可以正常打包，ICU 与多语言界面可用。已把错误的 `GTK_IM_MODULE=fcitx5` 改为 GTK3 实际模块 ID `fcitx`，并在启动时按 AppImage 挂载路径生成 Fcitx5 `immodules.cache`。内置终端候选按键 `^[[A` / `^[[D` 泄漏是否消除仍待 Linux 实机验证；详见 [remotedesktopmanager/README.md](./remotedesktopmanager/README.md)。
