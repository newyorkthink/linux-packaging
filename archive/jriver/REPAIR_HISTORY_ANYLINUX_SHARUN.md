# JRiver AnyLinux + quick-sharun 修复记录

本文件保留旧路线第 1～16 节的完整历史记录。第 1～13 节记录 AnyLinux + quick-sharun、CEF、网页音频、Fcitx5、glibc 隔离与实机结果；第 14～16 节是 2026-09-13 后 RunImage 路线的交叉记录。历史段落中的“当前入口”和旧文件名均表示**当时**的状态，不代表 2026-09-23 之后的 CI 入口。

2026-09-23 的正式入口是 `build_jriver_anylinux_sharun.sh`。当前状态和运行命令见 [README.md](./README.md)；RunImage 的完整修复记录见 [REPAIR_HISTORY_ANYLINUX_RUNIMAGE.md](./REPAIR_HISTORY_ANYLINUX_RUNIMAGE.md)。旧路线最后一次实机确认是 2026-09-12 GUI 可显示、影院模式鼠标点击卡住；此次重新接入后的新产物尚未实机验证。
...
> **2026-09-13 起正式入口：** `build_jriver.sh` 使用 **RunImage + quick-sharun**；新路线的流程、Actions 故障与修复记录已独立迁移至 [README_runimage_quick.md](./REPAIR_HISTORY_ANYLINUX_RUNIMAGE.md)。本 README 第 1～13 节只保留旧版稳定路线及实机历史。
>
...
按当前官方主程序名，将独立 `Build JRiver` Job 的 Release 资产从 `jriver.AppImage` 改为 `mediacenter36.AppImage`。只改外层文件名与 zsync 信息，不改旧版稳定路线的 CEF、音频、Fcitx5、glibc 隔离链，也不处理 Rofi 无 GUI。现行启动命令见 [README_runimage_quick.md](./REPAIR_HISTORY_ANYLINUX_RUNIMAGE.md) 第 8 节。

---

## 15. 2026-09-18：新资产终端 GUI 实机确认

`mediacenter36.AppImage` 从终端启动后 GUI 和简体中文界面正常。终端 Fontconfig 警告来自共享宿主字体配置，未阻止界面。Rofi 无 GUI 的停手结论见第 16 节。详见 [README_runimage_quick.md](./REPAIR_HISTORY_ANYLINUX_RUNIMAGE.md) 第 9、10 节。

---

## 16. 2026-09-18：Rofi 无 GUI —— 为什么改不了；不改 Rofi

完整证据、失败改动、禁止重复的方向在 [README_runimage_quick.md](./REPAIR_HISTORY_ANYLINUX_RUNIMAGE.md) 第 10 节。本节只把「改不了什么、为什么」写在本文件里，避免只看旧路线文档的人以为还能靠再改一版包装赌出窗口。

### 改不了的是什么

- **改不了的是：让 `mediacenter36.AppImage` 在仓库 `rofi.AppImage` 的 run 下稳定弹出主窗口。**
- **改得了、而且已经可用的是：终端直接启动。** GUI、简体中文界面、文件选择器已确认。这是当前正式用法。
- **不是 Rofi 坏了。** 同一套 Rofi 启动其他 AppImage 正常。**禁止改 `rofi/`。**
- **也不是这个 JRiver 包完全不能用。** 不能用的是 Rofi 这一条启动路径。

### 为什么包装再改也没用

Rofi 点下去之后，外层 AppImage、DwarFS、ssrv、包内 `mediacenter36` 都在，内层已经连上 `@/tmp/.X11-unix/X0`，但 `xwininfo` / `xlsclients` 找不到窗口。卡住时线程是：
...
这是已经 `exec` 进去的 GTK/Pango 初始化停住，不是入口脚本没跑。包装层（`AppRun`、`Run.rcfg`、`jriver-launch`）在 `exec` 之后没有控制权，拧入口和环境变量推不动这把用户态锁。

已经用实机排除、不要再当根因去改的：

1. 进程链没起来；
2. 没连上 X11；
3. Rofi 传入 tty 会话（覆盖成 `TERM=xterm-256color`、`XDG_SESSION_TYPE=x11` 后仍无窗）；
4. ssrv pipe 被 Fontconfig 警告写满（stdio 改接到文件后仍卡在 `futex_wait`，不是 `write`）；
5. 宿主字体共享 / Fontconfig 缓存（关掉共享并打进 CJK 字体、清缓存，仍同样卡住）；
6. 父级 Sharun 把 `LD_LIBRARY_PATH` / GIO / GTK 传进来（环境转储里没有这些变量；外层 AppRun 包装还自递归搞丢过终端 GUI）。

PATH 里见过 `/tmp/.mount_rofi…/bin`、`/tmp/.mount_i3…/bin`，日志里见过更早一次启动的 CEF `observer_list.h` / `JRWeb waitpid`。这两项 **不能当根因**，也不许据此把旧 CEF 补丁打进新入口。
...
- 禁止再加外层 AppRun 包装。
- 禁止再做父级 Sharun 环境大清扫。
- 禁止再关宿主字体或打进整包 CJK 字体赌一把。
- 禁止在没有新证据时把 GVFS、独立 D-Bus、`LD_PRELOAD`、TTY/后台等待写成根因。
- 没有 core dump、Pango 持锁栈、或能对比「终端成功 / Rofi 失败」并指向单一缺失文件/ABI 错误的 `strace` 之前，**不再改 `jriver/build_jriver.sh` 去碰 Rofi 启动。**

当前 `build_jriver.sh` 里残留的 `jriver-launch`（重定向 stdio、丢掉 mount PATH、覆盖 TERM/会话类型）是定位留下的，**没有实机证明能让 Rofi 出窗**。不要再往上叠。

旧版稳定路线记录仍只在本文件第 1～13 节，不并进新路线，也不为 Rofi 无窗重新启用。

---

## 17. 2026-09-23：旧路线重新接入 CI（待构建及实机验证）

- 历史核查：旧入口 `build_jriver_legacy_20260913.sh` 实际以 AnyLinux / Arch 环境运行 quick-sharun 与 appimagetool；`jriver/` 提交历史没有 linuxdeploy 调用。“linuxdeploy 旧版”是名称记忆偏差，不应成为照搬通用 linuxdeploy 步骤的依据。
- 修改文件：旧脚本改名为 `build_jriver_anylinux_sharun.sh`，正式 Build JRiver Job 与脚本清单改指向它；原 RunImage 脚本改名保留。历史代码仍按原来的基线 blob SHA 检查后生成带 CEF shutdown、音频和 preload 兼容修复的实际构建脚本。
- quick-sharun 兼容层继续区分旧 `AppDir/.preload` 与新 `AppDir/lib/sharun-preload` 布局，保持 `anylinux.so` → `jriver-cef-env.so` → `jriver-cef-shutdown-guard.so` 顺序；不固定旧版 quick-sharun 或 appimagetool，不绕过校验。
- 新增的资产名传递只读取本次唯一生成的 `dist/mediacenterN.AppImage` 并写入 `dist/release-name.txt`，供现有发布 Action 使用。没有改旧脚本的 JRiver、CEF、音频、Fcitx5、pathmap 和 glibc 启动链。
- 已完成静态检查；本次 Actions 和新产物未运行验证。2026-09-12 的 GUI 结果仅属于当时旧产物，不能保证当前滚动的 quick-sharun、上游 JRiver 和 CEF 仍完全兼容。影院模式卡住以及文件选择器的历史限制也不因切回入口自动消失。
- 上游包核对：2026-09-23 当前 AUR 配方选取官方 `MediaCenter-36.0.26-amd64.deb`；实际下载 SHA-256 与配方一致，DEB 的 Package / Version / Architecture 为 `mediacenter36` / `36.0.26` / `amd64`，包内 `JRWeb` 仍包含旧路线要求的 `cf102dc1148d9d20a4692c0fb19586c42c96f7c3` CEF API hash。历史音频闭包列出的十个 Debian DEB 地址均返回 HTTP 200。以上是输入与链接检查，不能替代完整 Actions 构建或实机功能验证；上游版本变化后仍须以脚本内的 ABI 守卫为准。
