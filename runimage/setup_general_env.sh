...
# 5. 安装通用开发环境和依赖
# 安装 GTK/Qt、Python、音视频、打印、蓝牙、输入法、字体、桌面集成及通用开发运行依赖
yay -S --noconfirm pulseaudio alsa-utils ibus gtk2 gtk3 gtk4 pulseaudio-alsa alsa-lib freetype2 wqy-microhei gnome-themes-extra adwaita-icon-theme dbus at-spi2-core pango fuse2 \
  libappindicator-gtk3 fontconfig cmake git openssl base-devel mesa noto-fonts-emoji gdk-pixbuf2 libpulse libxcomposite libxcrypt-compat libxrandr openssl-1.1 libtorrent alsa-plugins \
  postgresql-libs pyside6 python-attrs python-brotli python-cairo python-cffi python-cryptography python-defusedxml python-docutils python-fonttools python-jinja python-lxml gcc \
  python-lz4 python-mako python-opengl python-pillow python-pip python-psutil python-pymupdf python-pynput python-pyopenssl python-pyqt5 python-pyqt6 python-reportlab python-requests \
  python-setuptools python-simplejson python-tornado python-wheel python-typing_extensions python-wxpython qt5-tools qt6-charts qt6-multimedia libxau libxcursor libxdamage \
  ...
  shared-mime-info libxml2 webkit2gtk-4.1 libjpeg-turbo libtool libxslt glu xorg-mkfontscale gst-plugins-bad gst-plugins-ugly gst-libav lsb-release inetutils qt6-5compat \
  boost-libs ca-certificates-utils debugedit pkgconf binutils socat tesseract-data-eng tesseract-data-chi_sim qt5-svg qt6-svg libsecret gsettings-desktop-schemas
...
# gsettings-desktop-schemas：补充 GTK/GNOME 程序常用的 GSettings 桌面配置 schema，并会自动安装 dconf，无需单独添加 dconf
...
# 将以下运行参数追加到 Run.rcfg，使重新打包后的 general_env RunImage 默认使用这些设置
{
  ...
  # general_env 不启用 RIM_RUN_IN_ONE 和 RIM_WAIT_RPIDS_EXIT
  # 多个程序共用同一个容器时，如果最先启动程序所在终端被 Ctrl-C 中断或关闭，后续同容器 GUI 程序可能出现 write EIO、卡死或异常退出
  # 因此 general_env 保持每次启动使用独立容器，避免不同程序之间相互影响
...
# 7. 瘦身并生成最终 general_env RunImage
# 删除打包后不需要的缓存、开发文件及其他可安全移除内容，缩小最终镜像体积
rim-shrink --all

# 将当前 RunImage 环境重新打包为 general_env RunImage
rim-build general_env
