...
log "在官方 AppRun 外层恢复旧包已验证可用的中文/XCB环境"
...
pulse_resolved="$(awk '/libpulse\.so\.0 => / {print $3; exit}' "$VERIFY_DIR/qt-multimedia-ldd.log")"
pulse_glib_resolved="$(awk '/libpulse-mainloop-glib\.so\.0 => / {print $3; exit}' "$VERIFY_DIR/qt-multimedia-ldd.log")"
[[ -n "$pulse_resolved" ]] || die "Qt5Multimedia 没有解析 libpulse.so.0。"
[[ -n "$pulse_glib_resolved" ]] || die "Qt5Multimedia 没有解析 libpulse-mainloop-glib.so.0。"
[[ "$(readlink -f "$pulse_resolved")" == "$(readlink -f "$QT_VERIFY_LIBRARY_DIR/libpulse.so.0")" ]] || \
  die "Qt5Multimedia 没有解析到 XnView 包内 libpulse.so.0。"
[[ "$(readlink -f "$pulse_glib_resolved")" == "$(readlink -f "$QT_VERIFY_LIBRARY_DIR/libpulse-mainloop-glib.so.0")" ]] || \
